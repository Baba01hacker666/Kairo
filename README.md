...
## ✨ Major Features
- **Shared Entity Context System**: Flyweight Archetype optimization (`SharedEntityContext`) in `@kairo/ecs` allowing thousands of entities to share invariant properties in a single memory record (**5.56x faster batch iteration**, **18.1% heap memory savings**).
- **Downloadable Library & Module Packaging**: Exportable as ESM (`kairo.mjs`), CommonJS (`kairo.cjs`), UMD browser script bundle (`kairo.umd.js`), and TypeScript `.d.ts` declaration files via `npm run build:package`. See [`docs/MODULE_USAGE.md`](docs/MODULE_USAGE.md).
