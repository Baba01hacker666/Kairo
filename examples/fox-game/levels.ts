...
export const WORLD_NAMES: Record<number, { name: string; description: string; color: string }> = {
  1: { name: 'Emerald Meadow', description: 'Sunlit pastures introducing pressure plates, crates, and gates.', color: '#10b981' },
  2: { name: 'Crystal Caves', description: 'Subterranean caverns with light beams and rotatable mirrors.', color: '#3b82f6' },
  3: { name: 'Ancient Ruins', description: 'Overgrown ruins featuring moving platforms, keycards, and portals.', color: '#f59e0b' },
  4: { name: 'Celestial Peak', description: 'High-altitude floating islands with weight-balanced switches and rotating bridges.', color: '#8b5cf6' },
...
export function generateAllLevels(): LevelDefinition[] {
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    if (world === 1) {
      // Meadow: Crates & Pressure Plates & Doors
      ...
      // Caves: Light Beams & Mirrors & Laser targets
      elements.push({ type: 'laser_source', pos: [1, 3], rotation: 0 }); // points East
      ...
      if (levelInWorld >= 4) {
        elements.push({ type: 'crate', pos: [3, 3] });
      }
    } else if (world === 3) {
      // Ruins: Moving Platforms & Keycards & Teleporters
      ...
      // Peak: Rotating Bridges & Weight Plates
      ...
      // Shadow Grove: Combined Master Mechanics
    ...
    const hintMsg = `World ${world} Level ${levelInWorld}: ${
      world === 1 ? 'Push crates onto pressure plates to keep doors open!' :
      world === 2 ? 'Interact with mirrors to reflect laser beams onto target sensors.' :
      world === 3 ? 'Collect matching keycards and use teleporters to navigate.' :
      world === 4 ? 'Step on switches to rotate bridges into place.' :
      'Combine keycards, lasers, crates, and teleporters to unlock the exit portal!'
    }`;
