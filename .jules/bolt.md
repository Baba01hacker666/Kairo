...
**Learning:** During event registration and unregistration, particularly in hot paths like ECS component additions or physics contact tracking, using `Array.prototype.filter()` to remove event listeners creates unnecessary garbage collection churn by implicitly allocating new arrays on every call. However, naively swapping `filter()` for single-entry `findIndex/splice` introduces regressions: it silently changes `EventBus.off()` semantics (only removes the first duplicate handler instead of all of them), and in-place `splice()` on collections that are being iterated (like `KeyEventTrigger.handleKeyDown` iterating bound events) skips elements when handlers unbind during dispatch.
**Action:** Use a zero-allocation read/write compaction helper (`removeHandlerInPlace`) that removes ALL matching entries while preserving order and array identity. Keep shallow-copy iteration (`[...list]`) wherever handlers can mutate the collection mid-dispatch (`emit()`, `handleKeyDown`); full allocation-free dispatch iteration is unsafe because `on()` re-sorts the live listener array during emit, which would double-invoke or skip entries under index-based iteration.

## 2026-08-22 - [Physics Force Semantics, Pair-Key Injectivity & ECS Zero-Allocation Queries]
**Learning:** Passing an absolute world position to cannon-es `applyForce/applyImpulse` (which expects a point RELATIVE to the center of mass) injects spurious torque equal to `position x force` — bodies far from the origin spin wildly under thrust. Multiplicative pair keys (`a * P + b`) silently collide once cannon body IDs exceed the prime constant in long-running spawn/despawn sessions, producing phantom collision events. Rewinding `nextEntityId` in `World.clear()` lets stale retained entity handles alias onto brand-new entities. On the allocation side, `Object.keys()` per tween per frame and `query().slice()` per system per tick are steady GC churn on hot paths.
**Action:** Convert force application points to body-relative coordinates before handing them to cannon (and omit the argument entirely for center-of-mass forces) using module-scoped temp `Vec3`s. Replace pair keys with Cantor pairing `(sum*(sum+1))/2 + max` which is injective over all natural IDs. Never rewind entity ID counters; keep them monotonic across `clear()`. Add zero-allocation APIs following the existing optional-target pattern: `raycast(..., target?)`, `getVelocity(target?)`, `query(desc, out?)` / `queryEach(desc, fn)`, and hoist tween property-key arrays to construction time.

## 2026-08-22 - [Targeted Cache Invalidation, Resource Refcounting & Slot Recycling]
**Learning:** Wiping the entire query cache on every ECS mutation turns any per-tick spawner/component churn into O(queries x entities) rebuild storms with per-entity closure allocations. Blind GPU disposal of geometries/materials in `destroyEntity` corrupts any entity that shares resources with survivors. Spawn-only SoA buffers converge to scanning every slot ever used because dead slots below the high-water mark are never recycled. Allocating two CollisionEvent objects per contact pair per fixed step is constant GC churn in busy scenes. Stepping physics off the variable-dt `update` event diverges from gameplay systems that consume the fixed accumulator and silently time-dilates on hitches beyond MAX_SUBSTEPS.
**Action:** Track a revision counter per component type plus an entity-set revision; validate cached queries against the revisions they were built with so mutations only invalidate affected queries. Reference-count shared Three.js resources (`retainSharedResource`/`releaseSharedResource`) and dispose only at zero refs, keeping immediate disposal for untracked ones. Give FastSoAWorld a `despawnEntity(id)` free-list so spawn/despawn cycles reuse slots instead of growing the watermark. Pool module-scoped mutable CollisionEvent instances (listeners must copy out, never retain). Drive `physics.step(Time.fixedDeltaTime)` from the engine's `fixedUpdate` event.
## 2024-12-11 - [Eliminate Vector3 Allocations in Scripting Cinematic Camera Methods]
**Learning:** Functions like `cutToShot`, `panCamera`, and `craneShot` in `ScriptBehavior` were instantiating new `THREE.Vector3` objects on every call when passed array arguments `[x, y, z]`. This behavior is identical to what was found in `VideoEditor.ts` but had not been consistently applied across the codebase, creating unnecessary Garbage Collection spikes.
**Action:** Applied the zero-allocation `_setVector3` normalization helper pattern to `ScriptBehavior` and introduced pre-allocated target vectors (`_evalPos1`, `_evalPos2`, `_evalTarget`) to process cinematic shot commands without allocations. Also applied the `target` parameter pattern to `getPosition()`.
...
## 2024-12-11 - [Zero-Allocation Iteration in Shared Entity Context and Tweens]
**Learning:** During analysis of the SharedEntityContext system and Tween initialization loops, it became apparent that retrieving keys or values using `Object.keys()` or `Object.values()` creates implicit array allocations, increasing GC pressure. In SharedEntityContext, the keys were already safely cached at instantiation but the `getStats()` loop unnecessarily generated new arrays. In the Tween engine, copying values via `Object.keys` allocated an array per tween creation.
**Action:** When tracking object keys that are invariant after initialization, cache `Object.keys()` on the class constructor. For general iteration in game loops or frequent paths, prefer `for...in` combined with `Object.prototype.hasOwnProperty.call()` instead of using `Object.entries()`, `Object.keys()`, or `Object.values()`.
